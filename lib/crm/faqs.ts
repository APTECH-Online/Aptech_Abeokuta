...
export async function getFaqs(filter: FaqsFilter) {
  ...
  let query = supabase.from('faqs').select('*', { count: 'exact' }).order('sort_order', { ascending: true })
  ...
  if (error) {
    console.error('[crm] failed to load FAQs', error)
    return { items: [] as Faq[], total: 0, page, pageSize }
  }

  return { items: (data ?? []) as Faq[], total: count ?? 0, page, pageSize }
}
